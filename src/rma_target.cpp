...
static constexpr auto ringbuffer_length = 3U;
bool              quit    = false;
...
void handle_connected(rdma_endpoint& endpoint, const server_connection_v2& server_data, const app_options& cfg)
{
    static constexpr std::uint64_t SEND_COMPLETION_FLAGS = FI_SEND | FI_MSG;
    static constexpr std::chrono::milliseconds REMOTE_WRITE_TIMEOUT = std::chrono::seconds{2};
    static constexpr std::chrono::milliseconds SEND_TIMEOUT = std::chrono::seconds{2};

    std::vector<std::unique_ptr<char[], aligned_deleter<char>>> ringbuffer{ringbuffer_length};
    std::vector<std::unique_ptr<fid_mr>> mrs{ringbuffer_length};
    std::vector<memory_region_entry_v2> mr_list{ringbuffer_length};
    for (size_t i = 0; i < ringbuffer_length; ++i)
    {
        ringbuffer[i] = allocate_aligned<char>(sysconf(_SC_PAGE_SIZE), server_data.frame_size);
        memset(ringbuffer[0].get(), 0, server_data.frame_size);
        const int res = fi_mr_reg(endpoint.domain_.get(),
                                  ringbuffer[i].get(),
                                  server_data.frame_size,
                                  FI_REMOTE_WRITE,
                                  0,
                                  0,
                                  0,
                                  make_out_pointer(mrs[i]),
                                  nullptr);
        if (res != 0)
        {
        ...
        mr_list[i].address = (uintptr_t)ringbuffer[i].get();
        mr_list[i].size = server_data.frame_size;
        mr_list[i].rkey = fi_mr_key(mrs[i].get());
    }

    std::unique_ptr<fid_mr> mr_mr_list;
    int res = fi_mr_reg(endpoint.domain_.get(),
                        mr_list.data(),
                        sizeof(memory_region_entry_v2) * ringbuffer_length,
                        FI_SEND,
                        0,
                        0,
                        0,
                        make_out_pointer(mr_mr_list),
                        nullptr);
    if (res != 0)
    {
        throw rdma_error{"fi_mr_reg", res};
    ...
    res = fi_send(endpoint.endpoint_.get(),
                  mr_list.data(),
                  sizeof(memory_region_entry_v2) * ringbuffer_length,
                  fi_mr_desc(mr_mr_list.get()),
                  FI_ADDR_UNSPEC,
                  nullptr);
    if (res != 0)
    {
        throw rdma_error{"fi_send", res};
    }
    ...
    if (ret <= 0 || (entry.flags & SEND_COMPLETION_FLAGS) != SEND_COMPLETION_FLAGS)
    {
        std::cout << "Couldn't send MR list message\n";
        return;
    }

    // TODO check if there's OK??

    endpoint.receive_empty_message(); // Needed for write with imm data

    while (!stopped)
    {
        //std::cout << "Waiting on completion\n";
        ssize_t ret = fi_cq_sread(endpoint.completion_queue_.get(),
                                  &entry,
                                  1,
                                  nullptr,
                                  static_cast<int>(REMOTE_WRITE_TIMEOUT.count()));
        if (ret == 1)
        {
            std::cout << "Got completion! - flags:" << entry.flags << " data:" << entry.data << "\n";
            endpoint.receive_empty_message(); // Needed for write with imm data
            assert((entry.flags & (FI_RMA | FI_REMOTE_WRITE | FI_REMOTE_CQ_DATA)) == entry.flags);
        }
        else
        {
            throw rdma_error{"fi_cq_sread", (int)ret};
        }
    }
...
{
    auto fabric_info = get_fabric_info(cfg.provider_name, cfg.address, cfg.port, cfg.local_address, true);
    rdma_adapter  adapter{std::move(fabric_info)};
    rdma_endpoint ep{adapter};

    client_connection_v2 client_data;
    client_data.identifier        = PROTOCOL_IDENTIFIER_V2;
    client_data.ringbuffer_length = ringbuffer_length;
