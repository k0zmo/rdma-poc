...
void handle_connection(rdma_endpoint& endpoint, const app_options& cfg, const client_connection_v2& client_data)
{
    static constexpr std::chrono::milliseconds ACCEPT_TIMEOUT        = std::chrono::seconds{2};
    static constexpr std::chrono::milliseconds MR_LIST_RECV_TIMEOUT  = std::chrono::seconds{2};
    static constexpr std::chrono::milliseconds WRITE_TIMEOUT         = std::chrono::seconds{2};
    static constexpr std::uint64_t             RECV_COMPLETION_FLAGS = FI_RECV | FI_MSG;

    // Prepare a buffer to receive MR descriptors
    // TODO: times 2 for metadata
    std::vector<memory_region_entry_v2> mr_list{client_data.ringbuffer_length};
    std::unique_ptr<fid_mr> mr_list_mr;
    int res = fi_mr_reg(endpoint.domain_.get(),
                        mr_list.data(),
                        mr_list.size() * sizeof(memory_region_entry_v2),
                        FI_RECV,
                        0,
                        0,
                        0,
                        make_out_pointer(mr_list_mr),
                        nullptr);
    if (res != 0)
    {
    ...
    // Prepare for receiving MR descriptors just before accepting the connection
    ssize_t ret = fi_recv(endpoint.endpoint_.get(),
                          mr_list.data(),
                          mr_list.size() * sizeof(memory_region_entry_v2),
                          fi_mr_desc(mr_list_mr.get()),
                          FI_ADDR_UNSPEC,
                          nullptr);

    // Allocate our buffer that we gonna write to the remote client  
    auto buf = allocate_aligned<char>(sysconf(_SC_PAGESIZE), cfg.frame_size);
    std::memset(buf.get(), 0, cfg.frame_size);
    std::unique_ptr<fid_mr> memory_region;
    res = fi_mr_reg(endpoint.domain_.get(),
                    buf.get(),
                    cfg.frame_size,
                    FI_WRITE,
                    0,
    ...
    server_connection_v2 server_data{};
    ...
    // Wait for the connection to be accepted and established
    uint32_t   event;
    ...
    ret = fi_eq_sread(endpoint.event_queue_.get(),
                      &event,
                      cm_entry,
                      cm_entry_size,
    ...
    // Wait for mr_list to be received
    fi_cq_data_entry entry;
    ret = fi_cq_sread(endpoint.completion_queue_.get(),
                      &entry,
                      1,
                      nullptr,
                      static_cast<int>(MR_LIST_RECV_TIMEOUT.count()));
    if (ret <= 0 || (entry.flags & RECV_COMPLETION_FLAGS) != RECV_COMPLETION_FLAGS)
    {
        std::cout << "Didn't receive MR list message\n";
        fi_shutdown(endpoint.endpoint_.get(), 0U);
        return;
    }
    if (entry.len != mr_list.size() * sizeof(memory_region_entry_v2))
    {
        std::cout << "Received MR list message has different size than expected (" << mr_list.size() << ")\n";
        fi_shutdown(endpoint.endpoint_.get(), 0U);
        return;
    }

    // Validate mr_list
    for (auto& entry : mr_list)
    {
        if (entry.size != cfg.frame_size)
        {
            std::cout << "Received invalid MR list message\n";
            fi_shutdown(endpoint.endpoint_.get(), 0U);
            return;
        }
    }

    size_t          remote_mr_index    = 0;
    int             num_frames_written = 0;
    const uintptr_t client_id          = reinterpret_cast<uintptr_t>(&endpoint);

    // The beginning of the frame payload is this 32-bytes header
    auto* fbh = reinterpret_cast<frame_buffer_header*>(buf.get());
    fbh->usage_counter     = 10000 + num_frames_written;
    ...
    while (true)
    {
        res = fi_writedata(endpoint.endpoint_.get(),
                           buf.get(),
                           cfg.frame_size,
                           fi_mr_desc(memory_region.get()),
                           remote_mr_index,
                           FI_ADDR_UNSPEC,
                           mr_list[remote_mr_index].address,
                           mr_list[remote_mr_index].rkey,
                           nullptr);
        if (res != 0)
        {
            throw rdma_error{"fi_writedata", static_cast<int>(res)};
        }
        ...
        ret = fi_cq_sread(endpoint.completion_queue_.get(),
                          ...
                          1,
                          nullptr,
                          static_cast<int>(WRITE_TIMEOUT.count()));
        if (ret == 1)
        {
            remote_mr_index = (remote_mr_index + 1) % client_data.ringbuffer_length;
            num_frames_written += 1;
            if (cfg.verbose)
            {
                std::cout << client_id << ": Frame (" << num_frames_written << ", " << cfg.frame_size
        ...
        else if (ret == -FI_EAGAIN)
        {
            ...
            {
                std::cout << client_id << ": Received SHUTDOWN from the peer\n";
            }
            else
            {
...
{
    auto                    fabric_info = get_fabric_info(cfg.provider_name, cfg.address, cfg.port, true);
    ...
    while (true)
    {
        ...
        if (connection_data_size >= sizeof(client_connection))
        {
            ...
            if (client_connection_data.identifier == PROTOCOL_IDENTIFIER_V2)
            {
                if (connection_data_size >= sizeof(client_connection_v2))
                {
                    client_connection_v2 client_connection{};

                    std::memcpy(&client_connection,
                                entry->data,
                                std::min(sizeof(client_connection_v2), connection_data_size));

                    char flow_id[32 + 4 + 1];
                    std::sprintf(
                        flow_id,
                        "%02x%02x%02x%02x-%02x%02x-%02x%02x-%02x%02x-%02x%02x%02x%02x%02x%02x",
                        client_connection.flow_id[0],
                        client_connection.flow_id[1],
                        client_connection.flow_id[2],
                        client_connection.flow_id[3],
                        client_connection.flow_id[4],
                        client_connection.flow_id[5],
                        client_connection.flow_id[6],
                        client_connection.flow_id[7],
                        client_connection.flow_id[8],
                        client_connection.flow_id[9],
                        client_connection.flow_id[10],
                        client_connection.flow_id[11],
                        client_connection.flow_id[12],
                        client_connection.flow_id[13],
                        client_connection.flow_id[14],
                        client_connection.flow_id[15]);

                    std::cout << "Connection data:"
                              << "\n  Flow identifier: " << flow_id
                              << "\n  Ring buffer length: " << client_connection.ringbuffer_length
                              << std::endl;
                    ...
                    if (!std::strcmp(flow_id, "e569f502-8891-4c9f-92d4-51702b158bd5"))
                    {
                        try
                        {
                            std::thread th{[ep = rdma_endpoint{adapter, *entry->info},
                                            cfg,
                                            client_connection]() mutable -> void {
                                try
                                {
                                    handle_connection(ep, cfg, client_connection);
                                }
                                catch (const std::exception& ex)
                                {
                                    std::cout << "EXCEPTION from client thread: " << ex.what()
                                              << std::endl;
                                }
                            }};
                            th.detach();
                ...
                else
                {
                    error_message_stream
                        << "Received wrong private data size for V2 connection (expected "
                        << sizeof(client_connection_v2) << " but received "
