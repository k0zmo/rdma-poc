...
inline constexpr std::uint32_t PROTOCOL_IDENTIFIER_V2 = 2;
...
struct client_connection_v2
{
    ...
    std::uint8_t flow_id[16];
    /// 
    std::uint16_t ringbuffer_length;
};

struct server_connection_v2
{
    /// Size (in bytes) of the frame the server will send.
    std::uint32_t frame_size = 0;
    /// Size (in bytes) of the frame metadata the server will send.
